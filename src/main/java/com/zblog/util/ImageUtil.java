...
//	}
	...
	public String getPreFilePath(FileBean bean){
		String tmpFilePath = bean.getTmpImgPath() + bean.getNewImgName() + bean.getImgSign() + bean.getImgSize() + IMAGE_SIGN_POINT + bean.getImgType();
//		System.out.println(tmpFilePath);
		return formatPath(tmpFilePath);
	}
	
	/**
	 * added by damon.niu 2013-01-25
	 * 获得图片根目录地址
	 */
	...
	// 获得 WebRoot 下图片路径
	public static String getNewFilePath(String webRootPath, FileBean bean) throws ServletException {
        ...
        // 生成日期小时字符串
     	...
     	String newFilePath = webRootPath + sFilePath + File.separator + sDatePath;// + File.separator;

		return newFilePath;
	}
	
	// 生成原始图片文件存储路径，封装方法
	public static String getOldFilePath(FileBean bean){
		// 原始文件路径 由于先要生成原始图片，再由原始图片生成缩略图 - 临时变量
		String tmpFilePath = bean.getNewImgPath() + bean.getNewImgName() + IMAGE_SIGN_POINT + bean.getImgType();
		return formatPath(tmpFilePath);
	}
	...
	public static String getNewName(FileBean bean){
		// 文件名称
		String fileName = System.currentTimeMillis() + bean.getImgSign() + String.valueOf(RandomUtil.NextInt(10000, 99999));
